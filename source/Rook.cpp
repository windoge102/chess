...
Rook::Rook(int x, int y, SDL_Texture* p_tex) : Piece(x, y, p_tex) {
	select = false; move = false;
}
void Rook::setSelectT(vector<int> &c) {
	//if (select == true) {select = false; possibleMoves.clear(); }
	if (select == false) {select = true; setPossibleMoves(c); }
}
void Rook::setSelectF() {
	if (select == true) {select = false; possibleMoves.clear(); }
}
void Rook::setPossibleMoves(vector<int> &c) {
...
bool Rook::checkFreeSquare(vector<int> &c, int targetX, int targetY) {
	bool avail = true;
	for (unsigned int i = 0; i < c.size(); i += 2) {
